...
var writeToManifest = function(directory) {
    return lazypipe().pipe(gulp.dest, path.dist + directory).pipe(browserSync.stream, {
        match: '**/*.{js,css}'
...
gulp.task('jshint', function() {
    return gulp.src(['bower.json', 'gulpfile.js'].concat(project.js)).pipe(jshint()).pipe(jshint.reporter('jshint-stylish')).pipe(gulpif(enabled.failJSHint, jshint.reporter('fail')));
});
// ### Clean
...
gulp.task('wiredep', function() {
    ...
    return gulp.src(project.css).pipe(wiredep()).pipe(changed(path.source + 'styles', {
        hasChanged: changed.compareSha1Digest
    })).pipe(gulp.dest(path.source + 'styles'));
});
//Build jekyll on file changes not in dist
gulp.task('jekyll', function() {
    exec('jekyll build --source=site --destination=site/_site -q', function(err, stdout, stderr) {
        console.log(stdout);
        browserSync.reload();
    });
});
